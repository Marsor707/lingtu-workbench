...
- **下载完成后可以定位安装包了**：新增 `POST /api/update/reveal`，sidecar 用 `node:child_process` 调 `open -R`（macOS）/ `explorer /select`（Windows）/ `xdg-open`（Linux）在文件管理器中选中刚下载的更新包；前端在下载完成后自动调用一次，并保留「打开文件夹」按钮兜底。
- 原决策中"不新增任何 Tauri 命令"不变：这条能力走本地 HTTP，不依赖 Tauri IPC，浏览器形态下同样可用；当时判定不可行的依据是"页面调不到 Tauri"，而不是"没有可用的打开方式"。
- 接口只接受下载目录内且真实存在的文件，越界返回 400、文件缺失返回 404；定位失败只当次提示，不改变"已下载"这一事实（前端仍可复制路径手动安装）。
- 「更新包的打开/定位体验止步于复制路径」这一条作废。
