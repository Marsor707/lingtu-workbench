...
function readStoredAdvancedSettings(): AdvancedSettings {
  try {
    ...
    const layout = parsed.layout === '单图' || parsed.layout === '四宫格' || parsed.layout === '二宫格' || parsed.layout === '九宫格' ? parsed.layout : DEFAULT_ADVANCED_SETTINGS.layout
...
function App() {
  ...
  const retryJob = async (jobId: string) => {
    const response = await fetch(`${LOCAL_API_BASE}/api/jobs/${encodeURIComponent(jobId)}/retry`, { method: 'POST' })
    const body = await response.json() as ApiJob | ApiErrorBody
    if (!response.ok || !('id' in body)) throw new Error(('error' in body && body.error?.message) || '重试任务失败，请稍后再试')
    updateQueueFromJob(body)
    void refreshStats()
    // 重试继续复用原任务的 SSE 通道，页面关闭也不影响后端执行。
  ...
  const subscribeJob = (jobId: string): Promise<QueueStatus> => new Promise((resolve) => {
    ...
    ;['snapshot', 'progress', 'completed', 'failed', 'cancelled', 'job.created', 'job.progress', 'job.item.updated', 'job.completed', 'job.error'].forEach((eventName) => {
...
function QueuePage({ queue, setQueue, onRefresh, onCancel, onRetry, onCreate, onViewResults }: { queue: QueueItem[]; setQueue: Dispatch<SetStateAction<QueueItem[]>>; onRefresh: () => Promise<void>; onCancel: (jobId: string) => Promise<void>; onRetry: (jobId: string) => Promise<void>; onCreate: () => void; onViewResults: (jobId: string) => void }) {
  ...
  const handleRetry = async (jobId: string) => {
    ...
    setActionError('')
    setRetryingIds((ids) => new Set(ids).add(jobId))
    try {
      await onRetry(jobId)
    } catch (error) {
      setActionError(error instanceof Error ? error.message : '重试任务失败，请稍后再试')
    } finally {
      setRetryingIds((ids) => {
